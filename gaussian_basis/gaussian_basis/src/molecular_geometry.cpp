...
static Vec3 rotate(Vec3 r, double angle, Vec3 axis, Vec3 offset) {
    #define MUL(a, b)\
...
::MolecularGeometry(const std::map<AtomicSymbol, 
    ...
    for (auto &e: atom_positions) {
        ...
        std::vector<Vec3> positions = e.second;
        for (auto &position: positions)
            m_positions[int(atomic_symbol)].push_back(position);
    }
}
...
::add_atom(AtomicSymbol atomic_symbol, const Vec3 &position) {
    m_positions[int(atomic_symbol)].push_back(position);
}

std::vector<Vec3> MolecularGeometry
::operator[](AtomicSymbol atomic_symbol) const {
    return m_positions[int(atomic_symbol)];
}

MolecularGeometry MolecularGeometry
::operator+(const Vec3 &translate_position) const {
...
MolecularGeometry MolecularGeometry
::rotate(double angle, const Vec3 &rotation_axis, const Vec3 &offset) const {
    MolecularGeometry res{};
    for (int n = 1; n < m_positions.size(); n++) {
        for (auto &position: m_positions[n])
            res.m_positions[n].push_back(
...
molecular_geometry::MolecularGeometry make_oh(
    double oh_length, const Vec3 &axis
) {
    return MolecularGeometry({
        {AtomicSymbol::H, {oh_length*axis}},
        {AtomicSymbol::O, {{.x=0.0, .y=0.0, .z=0.0}}},
    });
}
...
molecular_geometry::MolecularGeometry
    ...
    return MolecularGeometry ({
        {AtomicSymbol::C, {{.x=0.0, .y=0.0, .z=0.0}}},
...
make_ch3(double ch1, double ch2, double ch3, double ach_angle,
    const Vec3 &axis1, const Vec3 &axis2) {
    Vec3 a{axis1}, x{axis2};
    Vec3 y = cross(a, x);
    Vec3 p1 = x;
    double phi = 2.0*(3.141592653589793)/3.0;
    Vec3 p2 = x*cos(phi) + y*sin(phi);
    Vec3 p3 = x*cos(2.0*phi) + y*sin(2.0*phi);
    return MolecularGeometry(
        {
            {AtomicSymbol::C, {{0.0, 0.0, 0.0}}},
            {AtomicSymbol::H, {
