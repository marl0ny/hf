...
uniform vec3 simulationDimensions3D;
// uniform ivec2 texelDimensions2D;
// uniform ivec3 texelDimensions3D;
// uniform ivec2 scatterDimensions2D;
...
void main() {
    vec3 coord3D = texture2D(scatterTex, position.xy).xyz;
    vec3 texCoord3D = coord3D/simulationDimensions3D;
    vec2 quadCoord = position.zw;
    quaternion r = scale*rotate(
        quaternion(2.0*(texCoord3D - vec3(0.5)), 0.0), rotation);
    vec4 finalPosition = r + quadScale*quaternion(quadCoord.xy, vec2(0.0));
    gl_Position
    ...
    gl_Position.z = gl_Position.z/100.0 - 0.01;
}