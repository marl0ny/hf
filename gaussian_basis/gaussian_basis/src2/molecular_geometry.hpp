...
namespace molecular_geometry {
    ...
    class MolecularGeometry {
        ...
        MolecularGeometry(
            ...
            const std::vector<std::vector<spatial::Vector>>
            &positions_list);
        MolecularGeometry(
            const std::map<AtomicSymbol,
            std::vector<spatial::Vector>>&);
        void add_atom(
            AtomicSymbol atomic_symbol,
            const spatial::Vector &position);
        std::vector<spatial::Vector>
            operator[](AtomicSymbol atomic_symbol) const;
        MolecularGeometry operator+(const spatial::Vector &position) const;
        MolecularGeometry operator-(const spatial::Vector &position) const;
        MolecularGeometry operator+(const MolecularGeometry &geom) const;
        MolecularGeometry rotate(
            double angle, const spatial::Vector &rotation_axis,
            const spatial::Vector &offset
                =spatial::Vector{.t=0.0, .x=0.0, .y=0.0, .z=0.0}) const;
        NuclearChargesArray get_nuclear_configuration() const;
        private:
        std::vector<std::vector<spatial::Vector>> m_positions;
    ...
    make_oh(
        double oh_length, 
        const spatial::Vector &axis=spatial::Vector{.x=0.0, .y=0.0, .z=1.0});
